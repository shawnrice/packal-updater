...
class Action {
	...
	public function __construct( $args ) {
		...
		$this->theme 		= isset( $args['theme'] ) ? $args['theme'] : false;
		$this->messages = [
			'status' => 'pass',
	...
	private function submit_theme( $theme ) {
		...
		$uri = $theme['uri'];
		// Now, we actually have to call the submit functions
	}

	private function submit_workflow( $workflow_path ) {
		$alphred = new Alphred;
		$alphred->console( 'DOING SUBMIT for ' . $workflow_path, 4 );
